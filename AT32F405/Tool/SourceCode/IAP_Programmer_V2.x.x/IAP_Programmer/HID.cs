...
{ 
    ...
    public class Hid : object
    {
        ...
        public HID_RETURN GetHIDDeviceList(UInt16 vID, UInt16 pID, out List<string> serialList)
        {
            ...
            for (int i = 0; i < deviceList.Count; i++)
            {
                IntPtr device = CreateFile(deviceList[i],
                                            ...
                                            DESIREDSHARE.FILE_SHARE_READ | DESIREDSHARE.FILE_SHARE_WRITE,
                                            0,
        ...
        /// <param name="vID">设备的vID</param>
        /// <param name="pID">设备的pID</param>
        /// <param name="bInterfaceNumber">设备的接口编号</param>
        /// <param name="serial">设备的serial</param>
        /// <returns></returns>
        public HID_RETURN GetHIDDeviceList(UInt16 vID, UInt16 pID, byte bInterfaceNumber, out List<string> serialList)
        {
            serialList = new List<string>();
            //获取连接的HID列表
            List<string> deviceList = new List<string>();
            ...
            {
                /* 1. 先按 mi_XX 过滤 */
                string path = deviceList[i].ToLowerInvariant();
                int miStart = path.IndexOf("&mi_");
                //if (miStart < 0) continue;                   // 不是目标接口
                if (miStart >= 0)
                {
                    string miStr = path.Substring(miStart + 4, 2); // 取 "01"
                    if (Convert.ToByte(miStr, 16) != bInterfaceNumber)
                        continue;
                }
        ...
        /// <param name="vID">设备的vID</param>
        /// <param name="pID">设备的pID</param>
        /// <param name="ver">设备的version</param>
        /// <returns></returns>
        public HID_RETURN GetHIDDevice(UInt16 vID, UInt16 pID, out UInt16 ver, out String deviceName)
        {
            ver = 0;
            deviceName = "";
            //获取连接的HID列表
        ...
        /// <param name="ver">设备的version</param>
        /// <returns></returns>
        public HID_RETURN GetHIDDevice(UInt16 vID, UInt16 pID, byte bInterfaceNumber, out UInt16 ver, out String deviceName)
        {
            ver = 0;
            deviceName = "";
            //获取连接的HID列表
        ...
        /// <param name="serial">设备的serial</param>
        /// <returns></returns>
        public HID_RETURN OpenDevice(UInt16 vID, UInt16 pID, byte bInterfaceNumber, string serial)
        {
            if (deviceOpened == false)
            {
                //获取连接的HID列表
        ...
        /// <param name="pID">设备的pID</param>
        /// <returns></returns>
        public bool CheckHidDeviceStatus(UInt16 vID, UInt16 pID)
        {
            var query = $"Select * FROM Win32_PnPEntiry WHERE PNPDeviceID LIKE '%VID_{vID:X4}&PID_{pID:X4}%'";
            using (var seracher = new ManagementObjectSearcher(query))
            {
                ManagementObjectCollection devices = seracher.Get();
                if (devices.Count == 0)
                {
                    return false;
                }
            ...
            if (hidInfoSet != IntPtr.Zero)
            {
                ...
                for (index = 0; index < MAX_USB_DEVICES; index++)
                {
                    ...
                    if (SetupDiEnumDeviceInterfaces(hidInfoSet, IntPtr.Zero, ref hUSB, index, ref interfaceInfo))
                    {
                        ...
                        {
                            if (IntPtr.Size == 8)
                                deviceList.Add(Marshal.PtrToStringAuto((IntPtr)((long)pDetail + 4)));
                            else
